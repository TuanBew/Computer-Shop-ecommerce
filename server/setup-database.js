// MongoDB setup script for computer-shop database

// Connect to database (run this with: node setup-database.js)
...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://mongo:27017/computer-shop';

// Data from JSON files embedded directly in the script
const usersData = [
  {
...
const reviewsData = []; // Empty array as per your data
...
const otpsData = []; // Empty array as per your data
const categoriesData = [];
const ordersData = [];
const cartsData = [];
const apikeysData = [];

// Collections configuration - mapping collection names to data
const collections = [
...
async function setupDatabase() {
  ...
  while (connectionAttempts < maxAttempts) {
    try {
      // Connect to MongoDB
      console.log(`Attempt ${connectionAttempts + 1}: Connecting to MongoDB at ${MONGODB_URI}`);
      ...
      for (const collection of collections) {
        try {
          ...
          const documentsToInsert = collection.data.map(doc => {
            ...
            // Handle nested IDs in arrays (like products in payments)
            if (newDoc.products && Array.isArray(newDoc.products)) {
              newDoc.products = newDoc.products.map(product => {
                ...
                if (typeof newProduct.productId === 'string') {
